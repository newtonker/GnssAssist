...
public class FileSelectView extends ListView implements OnItemClickListener
{
	...
	//每一次打开一路径后将其文件夹和文件分开保存到两个列表中
	private ArrayList<Map<String, Object>> folderList;
	...
	private void refreshFileList()
	{
		...
		mSimpleAdapter = new SimpleAdapter(getContext(),
				list, R.layout.filedialog_item,
				new String[]{"img", "name", "path"},
				new int[]{R.id.filedialog_item_img, R.id.filedialog_item_name, R.id.filedialog_item_path});
	...
	{
		//条目选择
		mFilePath = (String) list.get(position).get("path");
		mFileName = (String) list.get(position).get("name");
		//获取当前路径所表示的文件
		...
		{
			tempParent = tempFile.getAbsoluteFile().getParent();
			if(null != tempParent)
			{
				path = tempParent;
			}
