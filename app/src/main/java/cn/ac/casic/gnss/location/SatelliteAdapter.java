...
public class SatelliteAdapter
{
	...
	private MySatellite mMySatellite;
	...
	public void setSatelliteAdapter()
	{
		...
		mFirstFixTime = mGpsStatus.getTimeToFirstFix() / 1000;
		// 获取卫星信息列表
		mIterator = mGpsStatus.getSatellites().iterator();
		//初始化存储卫星列表
		...
		while (mIterator.hasNext() && mVisiable <= maxSatellites)
		{
			...
			//				mConnect++;
			//			}
			//			mGpsSatelliteList.add(mGpsSatellite);
			//			mVisiable++;
			...
			if(mFakeElevation > 90)
			{
				...
				mMySatellite = new MySatellite(mPrn, mGpsSatellite.getSnr(), mRealElev, mGpsSatellite.getAzimuth());
				mVisiable++;
				if(0 == mSys)
				{
					mGpsVisiable++;
			...
			else
			{
				...
				mMySatellite = new MySatellite(mPrn, mGpsSatellite.getSnr(), mFakeElevation, mGpsSatellite.getAzimuth());
				mVisiable++;
				mGpsVisiable++;
				mMySatellite.setmSystem("gps");
				if(mGpsSatellite.usedInFix())
