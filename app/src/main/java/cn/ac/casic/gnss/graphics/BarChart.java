...
public class BarChart extends SurfaceView implements Callback
{
	...
	private BarChartCallbacks mBarChartCallbacks = new BarChartCallbacks()
	{
		...
		{
			mSnrLayoutWidth = signalFragment.getmSnrLayoutWidth();
			mSnrLayoutHeight = signalFragment.getmSnrLayoutHeight();
	...
	public void drawCoordinate(Canvas canvas)
	{
		if(null != canvas)
		{
			...
			{
				canvas.drawLine(mBorderLeft, mBorderTop+(i-1)*mLineGap,
						mBorderRight, mBorderTop+(i-1)*mLineGap, mDashedPaint);
			}
		}
	}
	...
	public void drawBarChart(SatelliteAdapter satelliteAdapter, Canvas canvas)
	{
		...
		mGpsSatelliteSize = 0;
		isUsedInFix = false;
		mGpsSatelliteList = satelliteAdapter.getmGpsSatelliteList();
		...
		for (int i = 0; i < mGpsSatelliteSize; i++)
		{
			...
			canvas.drawRect(mBorderLeft + mBarGap * i,mBorderBottom - mSnr * mSnrGap,
					mBorderLeft+ mBarGap * (i + 1) - mBarChartSpacing, mBorderBottom, mChartPaint);
			// 信噪比值
